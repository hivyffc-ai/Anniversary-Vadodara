...
import { FFCBookingForm, FFCBookingFormInner, FFCWhatsAppFloat, FFCBookNowButton } from '@/components/ffc-booking-form';
...
export default function FFCServiceCategoryPage({ service }: ServiceCategoryPageProps) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-rose-800 via-amber-500 to-rose-900 text-white py-16 md:py-24">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-4 gap-8 max-w-5xl mx-auto">
            {content.process.map((step, index) => (
              <div key={index} className="text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-amber-500 to-amber-500 rounded-full flex items-center justify-center text-white text-2xl font-bold mx-auto mb-4">
                  {step.step}
                </div>
      ...
      <section className="py-16 bg-gradient-to-br from-amber-50 to-amber-50">
        <div className="container mx-auto px-4">
            ...
            {relatedPackages.map((pkg) => (
              <Link key={pkg.id} href={`/packages/${pkg.slug}`}>
                <Card className="h-full hover:shadow-xl transition-all hover:-translate-y-2 border-amber-100 overflow-hidden group">
                  <div className="aspect-[4/3] bg-gradient-to-br from-amber-200 to-amber-200 flex items-center justify-center">
                    <span className="text-6xl">{pkg.emoji}</span>
                  </div>
                  <CardContent className="p-4">
                    <h3 className="font-bold text-gray-800 mb-2 group-hover:text-rose-800 transition-colors line-clamp-2">
                      {pkg.name}
                    </h3>
                    <p className="text-sm text-gray-600 mb-3 line-clamp-2">{pkg.shortDescription}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-rose-800 font-bold">{formatPrice(pkg.price)}</span>
                      <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-semibold rounded bg-rose-800 text-white group-hover:bg-rose-900 transition-colors">View <ChevronRight className="h-3 w-3" /></span>
                    </div>
                  </CardContent>
                </Card>
              </Link>
          ...
          <div className="text-center mt-8">
            <Link href="/packages">
              <Button size="lg" className="bg-gradient-to-r from-rose-800 to-amber-500 hover:from-rose-900 hover:to-amber-600 text-white">
                View All 8 Packages
              </Button>
            </Link>
          </div>
      ...
      <section className="py-16 bg-amber-50">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="text-rose-800 text-sm">{content.testimonial.occasion}</div>
          </div>
        </div>
      </section>
            ...
            <Accordion type="single" collapsible className="space-y-4">
              {content.faqs.map((faq, index) => (
                <AccordionItem key={index} value={`faq-${index}`} className="border rounded-lg px-4">
                  <AccordionTrigger className="text-left font-medium text-gray-800 hover:text-rose-800">
                    {faq.question}
                  </AccordionTrigger>
      ...
      {/* Booking Form */}
      <section className="py-16 bg-gradient-to-br from-rose-800 to-amber-500">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
