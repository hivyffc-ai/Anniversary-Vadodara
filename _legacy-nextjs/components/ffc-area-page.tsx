...
export default function FFCAreaPage({ area }: AreaPageProps) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-rose-800 via-amber-500 to-rose-900 text-white py-16 md:py-20">
        <div className="container mx-auto px-4">
              ...
              {uniqueContent.topServicesInArea.map((service, index) => (
                <Card key={index} className="border-amber-200 hover:shadow-lg transition-all">
                  <CardContent className="p-6 text-center">
                    ...
                    <Badge className="mb-3 bg-amber-100 text-rose-900">{service.popularity}</Badge>
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              <article className="prose prose-lg max-w-none">
                  ...
                  <div className="bg-white rounded-xl p-6 mb-8 border border-amber-100">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {uniqueContent.nearbyLandmarks.map((landmark, index) => (
                        <span key={index} className="bg-amber-50 text-rose-900 px-3 py-1 rounded-full text-sm">
                          {landmark}
                        </span>
                      ))}
                    </div>
                ...
                {uniqueContent?.areaSpecialty && (
                  <div className="bg-gradient-to-r from-amber-100 to-amber-100 rounded-xl p-6 mb-8 border border-amber-200">
                    <h3 className="text-xl font-bold mb-3 text-rose-900">{uniqueContent.areaSpecialty.title}</h3>
                    ...
                    <div className="space-y-4">
                      {uniqueContent.popularOccasions.map((occ, index) => (
                        <div key={index} className="flex items-center justify-between border-b border-amber-100 pb-3 last:border-0">
                          ...
                          <Badge className="bg-amber-100 text-rose-900">{occ.percentage}</Badge>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {uniqueContent?.closingText && (
                  <div className="bg-gradient-to-r from-amber-50 to-amber-50 rounded-xl p-6 mb-8 border border-amber-200">
                  ...
                  {packages.slice(0, 4).map((pkg) => (
                    <Link key={pkg.id} href={`/packages/${pkg.slug}`}>
                      <Card className="h-full hover:shadow-lg transition-all hover:-translate-y-1 border-amber-100 group bg-white">
                        <div className="aspect-video bg-gradient-to-br from-amber-100 to-amber-100 flex items-center justify-center">
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-6">
                {/* Booking Form */}
                <FFCBookingFormInner pageTitle={`${area.name} Area`} />
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
            {nearbyAreas.map((nearbyArea) => (
              <Link 
                ...
                className="px-4 py-2 bg-amber-50 rounded-full text-gray-700 hover:bg-rose-800 hover:text-white transition-colors border border-amber-200"
              >
                {nearbyArea.name}
              </Link>
            ))}
