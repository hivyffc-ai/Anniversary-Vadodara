...
import { siteConfig, packages } from '@/lib/anniversary-config';
...
const ffcBookingSchema = z.object({
  ...
  occasion: z.string().min(1, 'Please select your anniversary type'),
...
const FORM_STORAGE_KEY = 'anniversary-booking-form-data';
...
// Anniversary occasion options
const momentOptions = [
  { value: 'wedding-anniversary', label: 'Wedding Anniversary' },
  { value: 'first-anniversary', label: 'First Anniversary (Paper)' },
  { value: '5th-anniversary', label: '5th Anniversary (Wood)' },
  { value: '10th-anniversary', label: '10th Anniversary (Tin)' },
  { value: '25th-anniversary', label: '25th Anniversary (Silver)' },
  { value: '50th-anniversary', label: '50th Anniversary (Golden)' },
  { value: 'dating-anniversary', label: 'Dating Anniversary' },
  { value: 'milestone-anniversary', label: 'Milestone Anniversary' },
  { value: 'other', label: 'Other Anniversary' },
];
...
interface FFCBookingFormInnerProps {
  pageTitle?: string;
...
export function FFCBookingFormInner({ pageTitle, variant = 'default', packageName, defaultPackageSlug, onClose }: FFCBookingFormInnerProps) {
  ...
  const generateWhatsAppMessage = (data: FFCBookingFormData): string => {
    ...
    let message = `*New Anniversary Booking Inquiry - Anniversary Celebration Vadodara*\n\n`;
    message += `*Your Name:* ${data.yourName}\n`;
    message += `*Partner's Name:* ${data.partnerName}\n`;
    message += `*Phone:* ${data.phone}\n`;
    message += `*City:* ${data.city}\n`;
    message += `*Anniversary Date:* ${data.occasionDate}\n`;
    message += `*Preferred Time:* ${timeLabel}\n`;
    message += `*Anniversary Type:* ${occasionLabel}\n`;
    ...
    message += `\nLooking forward to making your anniversary special!`;
      ...
      <Card className={`${variant === 'modal' ? '' : 'shadow-lg'}`}>
        <CardContent className="py-12 text-center">
          <div className="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center bg-amber-100">
            <CheckCircle className="h-8 w-8 text-rose-800" />
          </div>
  ...
  return (
    <Card className={`${variant === 'modal' ? 'border-0 shadow-none' : 'shadow-lg'} ${variant === 'hero' ? 'bg-white/60 backdrop-blur-md border-white/20' : ''} overflow-hidden`}>
      {variant !== 'modal' && (
        <div className={`${variant === 'hero' ? 'bg-gradient-to-r from-rose-800/95 to-amber-600/95' : 'bg-gradient-to-r from-rose-800 to-amber-600'} p-4 text-white`}>
          <CardTitle className="text-xl flex items-center gap-2">
            <Gift className="h-5 w-5" />
            Reserve Your Anniversary
          </CardTitle>
      ...
      {variant === 'modal' && onClose && (
        <div className="flex justify-between items-center p-4 border-b">
          <div>
            <CardTitle className="text-xl text-rose-800">Reserve Now</CardTitle>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="yourName" className="flex items-center gap-2">
              <User className="h-4 w-4 text-rose-800" />
              Your Name *
            </Label>
            <Input
              id="yourName"
          ...
          <div className="space-y-2">
            <Label htmlFor="partnerName" className="flex items-center gap-2">
              <Heart className="h-4 w-4 text-rose-800" />
              Your Partner&apos;s Name *
            </Label>
            <Input
              id="partnerName"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {packages.map((pkg) => (
                      ...
                      >
                        ...
                        <span className="w-4 shrink-0">
                          {selectedPackageSlug === pkg.slug && <Check className="h-3.5 w-3.5 text-rose-800" />}
                        </span>
                        {/* Package info */}
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-2 text-sm font-medium">
                <Gift className="h-4 w-4 text-rose-800" />
                Anniversary Type *
              </Label>
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-rose-800 to-amber-600 hover:from-rose-900 hover:to-amber-700 text-white py-5 text-base font-semibold mt-2"
          >
            {isSubmitting ? (
              <>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button className={`bg-gradient-to-r from-rose-800 to-amber-600 hover:from-rose-900 hover:to-amber-700 text-white ${className}`}>
...
// Controlled Dialog Booking Form - for use with external open/onOpenChange props
interface FFCBookingFormDialogProps {
  open: boolean;
...
export function FFCBookingForm({ open, onOpenChange, pageTitle, packageName, defaultPackageSlug }: FFCBookingFormDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md p-0">
        <FFCBookingFormInner 
          ...
          pageTitle={pageTitle} 
          packageName={packageName}
          defaultPackageSlug={defaultPackageSlug}
